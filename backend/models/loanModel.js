...
const getLoansByUser = async (userId) => {
    const [rows] = await db.query(
        `SELECT l.*,
                ...
                (l.principal_amount - IFNULL(SUM(p.amount), 0)) AS remaining_amount
         FROM loans l
...
const updateLoan = async (loanId, userId, data) => {
    ...
    const [result] = await db.query(
        `UPDATE loans SET borrower_name=?, borrower_contact=?, interest_rate=?, due_date=?, notes=?, status=?, updated_at=NOW()
...
const deleteLoan = async (loanId, userId) => {
    const [result] = await db.query('DELETE FROM loans WHERE id=? AND created_by=?', [loanId, userId]);
    return result;
};
...
const getSummary = async (userId) => {
    const [rows] = await db.query(
        `SELECT
            ...
            SUM(CASE WHEN type='given' THEN (principal_amount - IFNULL(paid,0)) ELSE 0 END) AS pending_receivable,
            SUM(CASE WHEN type='taken' THEN (principal_amount - IFNULL(paid,0)) ELSE 0 END) AS pending_payable,
            SUM(CASE WHEN status='overdue' THEN 1 ELSE 0 END) AS overdue_count
         FROM (
             SELECT l.id, l.type, l.principal_amount, l.status, IFNULL(SUM(p.amount),0) AS paid
             FROM loans l
             LEFT JOIN loan_payments p ON p.loan_id = l.id
             WHERE l.created_by = ?
             GROUP BY l.id
...
const markOverdueLoans = async (userId) => {
    await db.query(
        `UPDATE loans SET status='overdue' WHERE created_by=? AND due_date < CURDATE() AND status='active'`,
        [userId]
    );
};
